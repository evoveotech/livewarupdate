...
import { isWorldMonitorWebHost } from '@/services/runtime';
  ...
  renderLayout(): void {
    this.ctx.container.innerHTML = `
      ...
      <div class="header">
        ...
        const useInternal = this.ctx.isDesktopApp || location.hostname === 'localhost' || location.hostname === '127.0.0.1' || !isWorldMonitorWebHost(location.hostname);
        const inIframe = window.self !== window.top;
        const vHref = (v: string, prod: string) => useInternal || SITE_VARIANT === v ? '#' : prod;
        const vTarget = (v: string) => !useInternal && SITE_VARIANT !== v && inIframe ? 'target="_blank" rel="noopener"' : '';
        return `
          ...
          ${BRAND.showGitHubLinks ? `<a href="https://x.com/eliehabib" target="_blank" rel="noopener" class="credit-link">
      ...
      <nav class="mobile-menu" id="mobileMenu">
        ...
        ${BRAND.author ? `<a class="mobile-menu-item" href="https://x.com/${String(BRAND.author).replace('@', '')}" target="_blank" rel="noopener">
          ...
          <span class="mobile-menu-item-label">${BRAND.author}</span>
        </a>
        <div class="mobile-menu-divider"></div>` : ''}
        ${BRAND.footerLinks.length > 0 ? `<div class="mobile-menu-footer-links">
          ${BRAND.footerLinks.map((l) => `<a href="${l.href}" target="_blank" rel="noopener">${escapeHtml(l.label)}</a>`).join('\n          ')}
        </div>` : ''}
      ...
      <footer class="site-footer">
        <div class="site-footer-brand">
          ...
          <div class="site-footer-brand-text">
            <span class="site-footer-name">${escapeHtml(BRAND.siteName.toUpperCase())}</span>
            ${BRAND.footerTagline ? `<span class="site-footer-sub">${escapeHtml(BRAND.footerTagline)}</span>` : ''}
          </div>
        </div>
        ${BRAND.footerLinks.length > 0 ? `<nav>
          ${BRAND.footerLinks.map((l) => `<a href="${l.href}" target="_blank" rel="noopener">${escapeHtml(l.label)}</a>`).join('\n          ')}
        </nav>` : ''}
        <span class="site-footer-copy">&copy; ${new Date().getFullYear()} ${escapeHtml(BRAND.siteName)}</span>
      </footer>
    `;
